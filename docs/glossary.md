...
| 语言值 | `language` 字段的取值:`"auto"`(跟随系统)/ `"zh"` / `"en"` |
| `auto` 模式 | 语言偏好未显式指定时,启动时按系统 UI 语言(`GetUserDefaultUILanguage`)解析 |
| LCID | Windows 区域语言标识(16 位),低 10 位为主语言 ID;0x04=中文系,0x09=英文系 |
| 母语名 | 语言选项的显示写法,不随当前 UI 语言翻译:"简体中文"/"English" |
| i18n 表 | `src/i18n.rs` 中每个语言一个 `Strings` 结构体字面量,编译期保证字段齐全 |
| 托盘语言子菜单 | 托盘右键菜单中"语言 ▸"弹出子菜单,两项带互斥勾选 |
