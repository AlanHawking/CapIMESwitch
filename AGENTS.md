...
- **选项面板**: 托盘菜单「选项」打开设置窗口(类名 `CapIMESwitchOptionsWindow`)。开机启动勾选框与右键菜单共享 Run 键状态(保存时才写入,两个入口显示一致),提权模式下该菜单项置灰禁用;长按阈值 `long_press_ms` 存 exe 同目录 `config.toml`(范围 100-5000,默认 500,缺失/损坏回退默认);`run_as_admin` 布尔字段(默认 false)控制提权模式。点「保存」写入并即时生效,未保存关闭则丢弃;面板打开期间屏蔽托盘交互(`OPTIONS_OPEN` 标志)
- **提权/降权重启**: 保存开启提权模式时 `ShellExecuteW` runas 弹 UAC 并以 `--restart` 参数重启(取消则回滚配置);关闭时经 `/RL LIMITED` 一次性计划任务(`CapIMESwitchDeElevateTemp`)中转降权,由任务计划程序服务以普通令牌启动 `--restart` 实例(令牌 API 方案在 UAC 受限令牌下不可用);`--restart` 实例对单实例互斥体轮询重试(15s)等待旧实例退出,非提权实例启动时清理中转任务;启动时提权实例幂等确保计划任务存在
- **按键注入**: `INPUT` 结构合成 Win+Space(CapsLock 保持状态不发生)+ CapsLock toggle(硬件状态同步)

## 修改约束

- 修改键盘钩子逻辑 / 状态机时,必须同步更新 `caps.rs` 单元测试并跑通 `cargo test`
- 修改窗口类名 / 互斥体名 / 注册表值名 / 计划任务名(`CapIMESwitchElevated`)时,需同步检查 `smoke.ps1`、`setup.iss`(AppMutex、卸载清理)中的硬编码引用;修改 `config.toml` 字段时注意 `setup.iss` 的 `[UninstallDelete]`(卸载删 `config.toml`)与 `config.rs` 的 `#[serde(default)]` 兼容
...
- 手工验证:运行后短按/长按 CapsLock 观察输入法切换与 CapsLock LED;提权模式需人工走一遍「选项 → 以管理员身份启动 → 保存 → UAC 确认」,确认管理员 cmd 内 CapsLock 生效、`schtasks /Query /TN CapIMESwitchElevated` 可见任务,取消勾选后任务删除并降权